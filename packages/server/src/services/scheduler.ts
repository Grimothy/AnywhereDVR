...
export class Scheduler {
  ...
  private async matchSeriesRule(
    ...
    const existingRecordings = await db.recording.findMany({
      where: {
        ...
        status: { in: ['SCHEDULED', 'RECORDING', 'POST_PROCESSING', 'COMPLETED'] },
      },
  ...
  private async matchOnceRule(
    ...
    // Only match if program starts within lookahead window
    if (program.startTime < now || program.startTime > lookahead) return;
  ...
  private async matchManualRule(
    ...
    // Check if a recording already exists for this rule
    const existing = await db.recording.findFirst({
      where: {
        ruleId: rule.id,
        status: { in: ['SCHEDULED', 'RECORDING', 'POST_PROCESSING', 'COMPLETED'] },
