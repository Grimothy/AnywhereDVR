...
export default function ProgramDetailPanel({
  ...
  const hasSeasonPass = existingRules.some(
    (r) =>
      r.type === 'SERIES' &&
          ...
          <div className="flex items-center gap-3 mt-3">
            {recordStatus === 'success' ? (
              ...
              <>
                ...
                {hasSeasonPass ? (
                  <span className="px-4 py-1.5 bg-gold/10 border border-gold/30 text-gold rounded text-sm font-medium">
                    ✓ Season Pass active
                  </span>
                ) : (
                  <button
                  ...
                  >
                    {recordStatus === 'loading' ? 'Scheduling…' : 'Season Pass'}
                  </button>
                )}
              </>
            )}
